import { View, Text, TouchableOpacity, ActivityIndicator, FlatList, Switch, Appearance } from "react-native";
      ...
      <FlatList
          ...
          <View className="px-6 space-y-1">
            ...
            <View className="pt-6 pb-2">
              <Text className="text-sm font-semibold text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
                Blocked Users {blockedUsers && blockedUsers.length > 0 ? `(${blockedUsers.length})` : ""}
              </Text>
            </View>
          </View>
        }
        renderItem={({ item }) => (
          <View className="flex-row items-center px-6 py-3 border-b border-neutral-100 dark:border-neutral-800">
            ...
            <TouchableOpacity className="p-2" onPress={() => unblockUser(item._id)} disabled={isUnblocking}>
              {isUnblocking ? <ActivityIndicator size="small" color="#10B981" /> : <CheckCircle size={20} color="#10B981" />}
            </TouchableOpacity>
          </View>
        )}
        ListFooterComponent={
          <View className="px-6 pt-8 pb-12">
            <TouchableOpacity
              ...
              disabled={isPending}
            >
              {isPending ? <ActivityIndicator color="white" /> : <Text className="text-white font-semibold text-base">Log Out</Text>}
            </TouchableOpacity>
          </View>
        }
      />
